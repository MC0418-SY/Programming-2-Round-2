...
void problem_0d() {
    ...
    printf("\nValue using **ptrToPtr: %d", **ptrToPtr);
...
void problem_1() {
    ...
    Todo *todoPtr = (Todo *)malloc(sizeof(Todo));
    ...
    strcpy(todoPtr->description, "Master pointers");
...
void problem_2() {
    ...
    Todo *thirdTodoPtr = &(projectPtr->todos[2]);

    // Step 4: Use the new pointer to change the third todo's description
    // to "Updated Task" using strcpy
    strcpy(thirdTodoPtr->description, "Updated Task");

    // Step 5: Print to verify changes
    printf("\nFirst todo finished: %d", projectPtr->todos[0].isFinished);
    printf("\nSecond todo priority: %d", projectPtr->todos[1].priority);
    printf("\nThird todo description: %s\n", thirdTodoPtr->description);
...
void problem_4() {
    ...
    Todo *myTodo = (Todo *)malloc(sizeof(Todo));

    // Step 2: Allocate memory for one Project structure using malloc
    Project *myProject = (Project *)malloc(sizeof(Project));
...
void problem_5() {
    ...
    person->projects = malloc(INITIAL_CAPACITY * sizeof(Project));
...
void problem_8() {
    ...
    person.projects->todos = (Todo *)malloc(3 * sizeof(Todo));
...
void problem_9() {
    ...
    printf("\nProject names after update:\n");
    for (int i = 0; i < personPtr->projectCount; i++) {
        printf("Project %d: %s\n", i, personPtr->projects[i].projectName);
    }
...
void problem_21() {
    ...
    printf("\nBefore: Priority=%d, Finished=%d", myTodo.priority,
           myTodo.isFinished);
...
Todo createTodo(const char *desc, int priority, bool finished) {
    ...
    strcpy(newTodo.description, desc);
...
void problem_27() {
    ...
    person->projects = (Project *)malloc(INITIAL_CAPACITY * sizeof(Project));

    // Step 3: Set projectCapacity to INITIAL_CAPACITY
    person->projectCapacity = INITIAL_CAPACITY;

    // Step 4: Calculate new capacity (double the current capacity)
    int newCapacity = person->projectCapacity * 2;

    // Step 5: Use realloc() to resize the projects array to newCapacity
    // Store the result in a temporary pointer
    Project *newProjects =
        realloc(person->projects, newCapacity * sizeof(Project));
    ...
    if (newProjects != NULL) {
        ...
        person->projectCapacity = newCapacity;
    }
...
void insertLast(Project *project, Todo newTodo) {
    ...
    if (project->todoCount < project->todoCapacity) {
        // Step 2: Assign newTodo to the project's todos at index todoCount
        project->todos[project->todoCount] = newTodo;
...
void problem_30() {
    ...
    insertLast(&data.projects[targetProjectIndex], newTodo);

    // Step 3: Print the updated todo count to verify
    printf("\nProject %s now has %d todos",
           data.projects[targetProjectIndex].projectName,
           data.projects[targetProjectIndex].todoCount);
...
int main() {
    ...
    printf("Categories:\n");
    printf("0a-0e: Pointer Basics (Warm-up)\n");
    printf("1-3: Pointer Dereferencing\n");
    printf("4-6: Memory Allocation Basics\n");
    printf("7-8: Field Access & Manipulation\n");
    printf("9-11: Field Updates\n");
    printf("12-16: String Operations\n");
    printf("17-19: Structure Assignment\n");
    printf("20-25: Function Parameters\n");
    printf("26-27: Dynamic Memory Management\n");
    printf("28-31: Advanced Operations\n");
    printf("Run each function to test\n");
